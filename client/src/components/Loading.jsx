export default function Loading({ size = 'default', text = '' }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3">
      <div 
        className={`${sizeClasses[size]} border-fg/20 border-t-fg rounded-full animate-spin`}
      />
      {text && <p className="text-fg/60 text-sm">{text}</p>}
    </div>
  );
}
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="relative">
          <div className="w-16 h-16 border-4 border-fg/10 rounded-full" />
