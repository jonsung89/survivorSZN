...
export default function StatRankingDialog({ sport, statKey, statLabel, currentTeamIds = [], onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="p-4 flex items-center gap-3 border-b border-white/10 flex-shrink-0">
          <TrendingUp className="w-5 h-5 text-white/60" />
          <div className="flex-1">
            <h2 className="text-lg font-bold text-white">{statLabel} Rankings</h2>
            <p className="text-xs text-white/40">{SPORT_NAMES[sport] || sport.toUpperCase()} · League-wide</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <span className="text-sm text-white/40">Loading rankings...</span>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 px-4 py-2.5 transition-colors ${
                        isCurrentTeam ? 'bg-white/10' : 'hover:bg-white/5'
                      }`}
                    >
                      ...
                      <div className="flex items-center gap-2.5 flex-1 min-w-0">
                        ...
                        <span className={`text-sm truncate ${isCurrentTeam ? 'text-white font-semibold' : 'text-white/80'}`}>
                          {item.team.name}
                        </span>
                      </div>
                      <span className={`text-sm font-medium flex-shrink-0 ${isCurrentTeam ? 'text-white' : 'text-white/70'}`}>
                        {item.displayValue || '-'}
                      </span>
                    </div>
                  );
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <AlertCircle className="w-8 h-8 text-white/20" />
              <span className="text-sm text-white/40">No ranking data available</span>
            </div>
          )}
        </div>
      </div>
